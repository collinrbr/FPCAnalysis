...
isprecomputed = True
precomputedflnm = 'analysisfiles/tempvspos.pkl'
...
if(not(isprecomputed)):
    ...
    distdata = {'elecxxs':elecxxs,'elecyys':elecyys,'elechists':elechists,'elechistfac':elechistfac,'elechistboxfac':elechistboxfac,'vxelec':vxelec,'vyelec':vyelec,'vzelec':vzelec,'nx':nx,'ny':ny}
    picklefile = 'analysisfiles/tempvspos.pkl'
...
for _xidx in range(len(distdata['elecxxs'])):
    ...
    _xidxbval = ao.find_nearest(dfields['bx_xx'],distdata['elecxxs'][_xidx]) #Note: the input to ao.avg_dict should match the input used to create the loaded dataset, this is a quick approximate fix in case that is not true
...
def normalize_rightmost_nonzero(arr):
    ...
    if(rightmost_nonzero != None):
        normalized_arr = [val / rightmost_nonzero for val in arr]
    else:
        normalized_arr = [val for val in arr]
...
picklefile = 'analysisfiles/tempvspos_short.pkl'
with open(picklefile, 'wb') as handle:
...
os.system('mkdir figures/tempvspos_frame')

flnmprefix = 'figures/tempvspos_frame/'
